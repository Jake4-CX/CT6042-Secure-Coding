...
export class MessageController {
  ...
  async addMessage(request: Request, response: Response, next: NextFunction) {
    ...
    const message = await Database.getInstance().query("INSERT INTO messages (messageTitle, messageContent, userId) VALUES ('" + messageTitle + "', '" + messageContent + "', '" + user.id + "')");
  ...
  async updateMessage(request: Request, response: Response, next: NextFunction) {
    ...
    const messageCheck = await Database.getInstance().query("SELECT * FROM messages WHERE id = '" + id + "' AND userId = '" + user.id + "'");

    if (!messageCheck || messageCheck.length === 0) {
      return next(createError(401, "Message not found"));
    }

    const message = await Database.getInstance().query("UPDATE messages SET messageTitle = '" + messageTitle + "', messageContent = '" + messageContent + "' WHERE id = '" + id + "' AND userId = '" + user.id + "'");
  ...
  async deleteMessage(request: Request, response: Response, next: NextFunction) {
    ...
    const message = await Database.getInstance().query("DELETE FROM messages WHERE id = '" + id + "' AND userId = '" + user.id + "'");
  ...
  async getMessage(request: Request, response: Response, next: NextFunction) {
    ...
    const message = await Database.getInstance().query("SELECT messages.id, messages.messageTitle, messages.messageContent, " +
      " messages.createdAt, messages.updatedAt, users.id AS userId, users.userName, users.createdAt AS userCreatedAt, " +
      " users.updatedAt AS userUpdatedAt FROM messages JOIN users ON messages.userId = users.id WHERE messages.id = '" + id + "'");
